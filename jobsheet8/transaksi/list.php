...
<main><section>
	...
	<?php if (!$transaksi): ?><tr><td colspan="7" class="text-center">Belum ada transaksi.</td></tr><?php else: ?><?php foreach ($transaksi as $index => $item): ?><tr><td><?= $index + 1 ?></td><td><?= htmlspecialchars($item['pelanggan']) ?></td><td><?= htmlspecialchars($item['nama_paket']) ?></td><td><?= htmlspecialchars($item['berat']) ?> kg</td><td>Rp<?= number_format((int) $item['total'], 0, ',', '.') ?></td><td><?= htmlspecialchars($item['status']) ?></td><td><button type="button" class="btn btn-outline-primary btn-sm btn-cetak-struk"><i class="bi bi-receipt" aria-hidden="true"></i> Cetak Struk</button></td></tr><?php endforeach; ?><?php endif; ?>
	</tbody></table></div>
</section></main>
